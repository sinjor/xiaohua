...
drop table if exists cid_apply_derived_3m_hf;


create table cid_apply_derived_3m_hf as
select t1.event_cid,
       count(t1.event_cid) as cid_apply_3m_hf
from
    (select event_cid,
            collector_tstamp
...
/* 4.手机号短期多次获额 */
drop table if exists mobile_apply_derived_3m_hf;


create table mobile_apply_derived_3m_hf as
select t3.event_cid,
       t3.event_mobile,
       t4.mobile_apply_3m_hf
from
    (select event_cid,
            event_mobile,
            collector_tstamp
...
left outer join
    (select t1.event_mobile,
            t1.collector_tstamp,
            count(t2.event_mobile) as mobile_apply_3m_hf
     from
         (select event_cid,
                 event_mobile,
                 collector_tstamp
...
alter table cid_apply_derived_3m_hf add index index_event_cid (event_cid);
alter table cid_bind_bankcardno_derived_3m_hf add index index_event_cid (event_cid);
alter table mobile_apply_derived_3m_hf add index index_event_cid (event_cid);
alter table mobile_bind_bankcardno_derived_3m_hf add index index_event_cid (event_cid);
alter table certno_bind_cid_derived_3m_hf add index index_event_cid (event_cid);
alter table bankcardno_bind_cid_derived_3m_hf add index index_event_cid (event_cid);
...
select t0.event_cid,
       ifnull(t1.cid_apply_3m_hf, 0) as cid_apply_3m_hf,
       ifnull(t2.cid_bind_bankcardno_3m_hf, 0) as cid_bind_bankcardno_3m_hf,
       ifnull(t3.mobile_apply_3m_hf, 0) as mobile_apply_3m_hf,
       ifnull(t4.mobile_bind_bankcardno_3m_hf, 0) as mobile_bind_bankcardno_3m_hf,
       ifnull(t5.certno_bind_cid_3m_hf, 0) as certno_bind_cid_3m_hf,
       ifnull(t6.bankcardno_bind_cid_3m_hf, 0) as bankcardno_bind_cid_3m_hf
from
    (select event_cid
     from cid_first_loan) t0
left outer join cid_apply_derived_3m_hf as t1 on t0.event_cid = t1.event_cid
left outer join cid_bind_bankcardno_derived_3m_hf as t2 on t0.event_cid = t2.event_cid
left outer join mobile_apply_derived_3m_hf as t3 on t0.event_cid = t3.event_cid
left outer join mobile_bind_bankcardno_derived_3m_hf as t4 on t0.event_cid = t4.event_cid
left outer join certno_bind_cid_derived_3m_hf as t5 on t0.event_cid = t5.event_cid
left outer join bankcardno_bind_cid_derived_3m_hf as t6 on t0.event_cid = t6.event_cid;
